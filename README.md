...
and the lifecycle and startup components take `ZeroZero.Primitives` for the log sink. Taking the
MQTT module therefore brings three foundation assemblies with it and no brand assembly; an
application that wants the About control takes the brand component as well.
...
`ZeroZero.Primitives` and `ZeroZero.Win32` reference nothing at all; `ZeroZero.Controls.WinUI`
references the Windows App SDK alone. Every third-party version is pinned once, in the build kit's
