...
| Startup | `startup` | `ZeroZero.Startup` | The application's scheduled tasks. The logon task: its identity, the power-safe elevated definition, registration, the direct enabled read, enable, disable, delete, repair of a task an older build registered, and a demand start that proves the task runs. The watchdog task: the probe that starts the application again when its process is gone, on an interval and again just after an unlock and a resume, written and kept correct on every start, with the record of a deliberate exit that holds it off. The manifest, the installer and the single-instance gate stay with the application. Entry point `ZeroZero.Startup`; no user interface. | [`docs/zerozero-startup.md`](docs/zerozero-startup.md) |
...
| Update | `update` | `ZeroZero.Update`, `ZeroZero.Update.Win32`, `ZeroZero.Update.WinUI` | The update flow: the latest GitHub release against the running version, the download into a fresh private directory, verification of the installer before it runs — its Authenticode signature and publisher against the expected signer, and its SHA-256 against the hash the release publishes — the launch and the hand-over to the application's own shutdown, the stale-download sweep and the check scheduler; and one window in the studio's own style for every step a person sees. The options, the installer and when the application exits stay with the application. Entry point `ZeroZero.Update.WinUI`; an application with a surface of its own takes `ZeroZero.Update.Win32`, and a headless consumer `ZeroZero.Update`. | [`docs/zerozero-update.md`](docs/zerozero-update.md) |

| Foundation | Key | Package | What it is | Guide |
|---|---|---|---|---|
| Config | `config` | `ZeroZero.Config`, `ZeroZero.Config.Sections`, `ZeroZero.Config.Watch` | Settings on disk. The plain assembly is one JSON file holding one type: typed snapshot reads, mutation under one lock, a write that lands whole or not at all, change notification, quarantine of a file that cannot be parsed. The sections assembly is one document whose top-level keys are sections owned by different components, addressed one at a time — a sibling section, a section from a build that no longer exists and a hand-written comment all survive a write untouched, and no write ever adds a comment or a second key differing from another only in case — plus the migration from an older file, which writes the new one and leaves the old one alone. The watch assembly picks up an edit made outside the application without a restart: one file watcher, a quiet window so a single save is examined once, and a classifier naming which values are only cosmetic — everything it does not name counts, so a property added later cannot silently stop the reload firing. The MQTT module stores its settings and its discovery ledger through the plain assembly. Entry point `ZeroZero.Config.Sections`; a consumer whose file holds one type takes `ZeroZero.Config`, and one that wants hand edits picked up adds `ZeroZero.Config.Watch`. | [`docs/zerozero-config.md`](docs/zerozero-config.md) |
...
boxes, and `ZeroZero.Controls.WinUI` and the diagnostics, lifecycle, startup and update components
take `ZeroZero.Primitives` — for the log sink, and in diagnostics and update for the version reader
as well. Taking the MQTT
...
reference nothing at all, and `ZeroZero.Config.Sections` and `ZeroZero.Config.Watch` reference only `ZeroZero.Config` beneath
...
App SDK, the toolkit and `ZeroZero.Primitives`, and no toolkit type reaches its public signature;
...
plain `net10.0` assemblies and every test project but eight are portable in isolation.
...
personalisation key, `ZeroZero.Update.Tests` calls wintrust and signs files through PowerShell,
`ZeroZero.ReleaseVerification.Tests` reads Authenticode signatures through PowerShell, and
`ZeroZero.Config.Watch.Tests` asserts how many notifications one save delivers and in what shape,
which is a property of the Windows directory-change API and not of the assembly under test, so those
eight run on Windows only. The MQTT module additionally needs an MQTT 5.0 broker at run time, and
...
| [`docs/zerozero-config.md`](docs/zerozero-config.md) | The config foundation assemblies: the single-type settings file and its atomic write, the section-addressed document and what a write does and does not touch, the migration, and the watcher with its quiet window and change classifier. |
| [`docs/zerozero-controls.md`](docs/zerozero-controls.md) | The controls foundation assembly: the settings-row vocabulary, title-bar theming, the text prompt, the popup sized to its content, and how the harness shows each. |
...
| [`docs/zerozero-startup.md`](docs/zerozero-startup.md) | The startup component: the logon task and the watchdog task, their definitions and repair, what stays with the application, and the token each needs. |
