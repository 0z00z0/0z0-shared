...
`ZeroZero.Controls.WinUI` for the info bubble, the About window takes `ZeroZero.Win32`, and
`ZeroZero.Controls.WinUI` and `ZeroZero.Tray` take `ZeroZero.Win32` for their monitor and taskbar
metrics. Taking the MQTT module therefore brings four foundation assemblies with it and no brand
assembly; an application that wants the About control takes the brand component as well.
...
reference nothing at all; `ZeroZero.Controls.WinUI` references the Windows App SDK and the
toolkit, and no toolkit type reaches its public signature. Every third-party version is pinned
once, in the build kit's `ZeroZero.Packages.props`, which this repository's
`Directory.Packages.props` imports and every consuming repository imports the same way.
