...
`ZeroZero.Brand.WinUI`. So `primitives`, `config`, `win32` and `build` release in any order,
`brand` releases after `win32`, and `mqtt` releases after `primitives`, `config` and `brand` — last
of all. Within a component the order does not matter: the projects release together. The build kit
packs no assembly — its package is the MSBuild files, the manifest template and the signing script —
and the pack step counts it like any other project of its key.
...
`config`, `primitives` and `win32` packages at whatever version each last released.
