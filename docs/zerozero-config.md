...
desktop, no broker, no network. The migration's own proof runs against
`tests/ZeroZero.Config.Sections.Tests/Fixtures/awkward-settings.json`, a settings file carrying every
awkward property an installed one has. The watcher's tests change real files on a real disk and read
what the watcher reports; only the clock is substituted.
