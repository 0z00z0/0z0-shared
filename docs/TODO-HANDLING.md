# ToDo handling (studio-wide convention)

How work is tracked across ZeroZero Software (0z0) repos. Short version: **GitHub Issues are the
source of truth for each repo; a local git-ignored `TODO.md` mirrors them for at-a-glance/offline
work; the two are kept in sync both ways.**

This convention applies to every 0z0 repo. Each repo's own `TODO.md` header carries that repo's
specific `area:*` label list (its subsystems); everything else below is shared.

## Two surfaces

| Surface | Role | Committed? |
|---|---|---|
| GitHub Issues (the repo's own) | **Source of truth.** Every real task is an issue, labelled per the taxonomy below. | n/a (GitHub) |
| `TODO.md` | **Local mirror** — a grouped, tagged snapshot for quick scanning and offline planning. | No — git-ignored, local-only. |
| `TODO-HANDLING.md` (this file, in `0z0-shared`) | The convention itself, shared by all repos. | Yes. |
...
**Type** (what kind of work — same across all 0z0 repos)
...
**Area** (what part of the app — repo-specific)
Each repo defines its own `area:*` labels for its own subsystems. See that repo's `TODO.md` header
for its current list (e.g. ChargeKeeper uses `area:tray`, `area:settings`, `area:graph`, `area:mqtt`,
`area:vendor`, `area:network`, `area:core`, `area:installer`).

**Status** (optional, same across all 0z0 repos)
...
maps to the tracker. See a repo's commit history for examples.
