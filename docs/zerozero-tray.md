...
- **The menu's theme.** The menu is drawn in the taskbar's theme, read from the same registry value
  ...
  `FlushMenuThemes`, both exported by ordinal from `uxtheme` — rather than from a theme property on
  the flyout or the icon, which the menu does not read. The mode is process-wide, so it covers every
  Win32 menu the application shows; on a Windows without the export the menu stays as Windows draws
  it.
...
- **The icon's identity.** The icon registers under `Id` — what the application supplied, or what
  the host derived from `Name`. `ShellId` is that identity after `Start()`, and anything that asks
  the shell about the icon takes it: `Shell_NotifyIconGetRect`, and the per-icon settings below.
  The shell treats a change of identity as a new icon, so an identity kept fixed across versions is
  an icon that keeps where the person put it.
- **Placement.** `Placement` says whether the shell draws the icon in the notification area or
  behind the overflow chevron, and is null while the shell keeps no entry for it.
  `AskForPlacement(TrayIconPlacement)` writes the setting and registers the icon again, returning
...
### Where the shell draws the icon

The shell keeps one entry per icon under the current user's `Control Panel\NotifyIconSettings`,
keyed on the identity `ShellId` names, and the entry's `IsPromoted` value decides whether the icon
sits in the notification area beside the clock or behind the overflow chevron. It is the same value
the taskbar settings page writes, and it belongs to the person using the machine.

**A placement takes effect at the next sign-in.** The shell keeps its own list of the notification
area for the length of a session and writes that list out rather than reading it back. An
application that starts at logon therefore gets the placement it asked for; one switched on while
the session is running does not move until the next sign-in, so say that where the person chooses
the setting.

`TrayIconPlacements.Plan` in the plain assembly decides what to write, apart from the writing, and
`NotifyIconSettings` carries it out. The rules:
...
- Nothing is written when no entry carries the identity — the entry appears the first time the
...
process from outside, through the handle the test holds: the power-throttling state has no
execution-speed bit, with the icon created, so the argument the host passes is proved by the process
rather than by the argument. Priority class is not read there — it is inherited from whatever
launched the run, so a test host below normal produces a child below normal whatever the host does,
and the reading would measure the launcher. The other two read the probe — that the icon was
rendered at the taskbar's own slot and theme, and that an icon the application wrote itself is
created from its path. All three skip, with the reason, where the Windows App Runtime the
