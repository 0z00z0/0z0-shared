...
public class MqttModuleTests
{
    ...
    public void VersionIsWhatTheLoadedAssemblyCarriesRatherThanACompiledInConstant()
    {
        ...
        // Versions.props passes nothing here the moment the two differ, which is exactly the state a
        // consumer building from a working tree between tags is in.
    ...
    [Fact]
    public void VersionIsTheModulesOwnAndNotTheReadersOrTheHosts()
    {
        // The reader answers for any assembly; the module's property must ask about the module.
        Assert.Equal(AssemblyVersionText.Read(Module), MqttModule.Version);
        Assert.NotEqual(AssemblyVersionText.Read(typeof(MqttModuleTests).Assembly), MqttModule.Version);
    }
